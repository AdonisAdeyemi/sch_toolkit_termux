...
alert("picker.js says hiiiii");
 ...
 const GET_ENDPOINT = `/${appName}/api/compilation/get_items`;
  ...
  function short(s, n=140){ if(!s) return ''; return s.length>n? s.slice(0,n-1)+'…' : s; }
...
if (item.has_image) {
    ...
    var rawName = item.image_url //refactor - remove item.image_filename from db;
    var imgUrl;
      ...
      const card = $(`
        <div class="picked-item picked-highlight mb-2 p-2 ">
          <div class="d-flex justify-content-between">
            <div><strong> ${serialNumberSpan} ${escapeHtml(short(q.question_text || q.question || ''))}</strong>
...
reloader from ls : only for new compilation (empty array) ::: user of lsKeyvwt comp-id
 ...
 state.compilation_id = compilation_id ;
  ...
  function updateProgressBar(){
    ...
    let max_pick = 50; //myb later - regular will less eg 40 max 
    let total = max_pick;
