...
//xxxxxxxxxxxxxxxxx
function getSubjectId($pdo, $subjectName) {
    static $cache = [];

    $subjectName = strtolower(trim($subjectName));

    // 1. Check memory cache (VERY important for speed)
    ...
    return $cache[$subjectName] = $pdo->lastInsertId();
}
...
$subject_id = getSubjectId($pdo, $subject_input);
...
for ($currentPage = $startPage; $currentPage <= $stopPage; $currentPage++) {
    ...
    foreach ($questions as $i => $q) {
      ...
      $topic = ucfirst(strtolower(trim($topic)));
...
$stmt = $pdo->prepare("
    ...
    (exam_body, year, subject_id, topic, type, q_order, q_label, question_text, options, answer, has_image, image_url) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
");
...
$stmt->execute([
    $exam_body, 
    $year, 
    $subject_id,   // ✅ THIS is the key change
    $topic, 
    $type, 
    $q_order, 
    $q_order, 
